...
function rawMaker(labels, digits) {
  ...
  for (let i in labels) {
    ...
    let range = digits.slice(start, start + imageSize)
      // .map(normalize);
 ...
 */
function rawWriter(raw, prefix, digitsDir = "./data/digits/fashion-mnist") {
  /**
...
module.exports = async function convert() {
  await Promise.all([
    digitsLoader("./data/fashion-mnist/train-images-idx3-ubyte"),
    labelsLoader("./data/fashion-mnist/train-labels-idx1-ubyte")
  ]).then(([digits, labels])=> {
    rawWriter(rawMaker(labels, digits), "training");
  });
  await Promise.all([
    digitsLoader("./data/fashion-mnist/t10k-images-idx3-ubyte"),
    labelsLoader("./data/fashion-mnist/t10k-labels-idx1-ubyte")
  ]).then(([digits, labels]) => {
    rawWriter(rawMaker(labels, digits), "test");
  });
};
